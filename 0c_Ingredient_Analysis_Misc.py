...
# -*- coding: utf-8 -*-
"""
Created on Thu Feb 22 09:06:27 2018
...
folders = ["Scraped_Recipes\Scrape_Results_Misc"]
...
index_list = list(range(1,115))
del index_list[87]     
# Load in the serving sizes 
for i in index_list:
    path = os.getcwd() + "/" +  folders[0]
    # And load in the serving sizes
    filename2 = path + '/servings_' + str(i) + '.txt'
    with open(filename2) as f:
        text = f.read()
        servings.append(text.strip())
        
# Remove what corresponds to 88 because it's empty
...
for i in index_list:
    path = os.getcwd() + '/' + folders[0] 
    filename = path + '/ingredients_' + str(i) + '.txt'
...
for i in index_list:
    path = os.getcwd() + '/' + folders[0]
    # And load in the serving sizes
    filename2 = path + '/rating_' + str(i) + '.txt'
    with open(filename2) as f:
        line = f.read()
        line = line.strip()
        orig_num = re.findall('(\d+[\/\d. ]*|\d)', line)
